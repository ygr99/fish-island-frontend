 ...
 * 哔哩哔哩随机视频组件
 */
class BilibiliRandomVideo {
  constructor() {
    this.name = "随机哔哩哔哩";
    this.iconClass = "fa-solid fa-video";
    this.backgroundColor = "bg-pink-400";

    // 添加新属性，用于组件库显示
    this.icon = "fa-solid fa-video";
    this.bgColor = "bg-pink-400";
    this.description = "随机跳转一个哔哩哔哩视频";
  }
  ...
  render() {
    return `
      <div class="app-container flex flex-col items-center cursor-pointer" id="bilibili-random">
  ...
  init() {
    document
      .getElementById("bilibili-random")
  ...
  async handleClick() {
    try {
      ...
      // 调用API获取随机视频
      const response = await fetch("/api/bilibili/random");
      ...
      this.showToast("获取视频失败，请重试", "error");
    }
  }
...
// 导出组件
window.BilibiliRandomVideo = BilibiliRandomVideo;
