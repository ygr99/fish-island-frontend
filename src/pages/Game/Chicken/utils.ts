...
// local
export const LAST_LEVEL_STORAGE_KEY = 'chicken_last_level';
export const LAST_SCORE_STORAGE_KEY = 'chicken_last_score';
export const LAST_TIME_STORAGE_KEY = 'chicken_last_time';
export const PLAYING_THEME_ID_STORAGE_KEY = 'chicken_playing_theme_id';
export const CUSTOM_THEME_STORAGE_KEY = 'chicken_custom_theme';
export const CUSTOM_SYSTEM_THEMES_STORAGE_KEY = 'chicken_custom_system_themes';
...
export const randomString: (len: number) => string = (len: number) => {
...
export const wrapThemeDefaultSounds: (theme: Theme<any>) => void = (theme) => {
  ...
  if (hasUseDefaultClick && !theme.sounds.find((s: any) => s.name === 'button-click')) {
    const defaultClick = defaultTheme.sounds.find((s: any) => s.name === 'button-click');
    // eslint-disable-next-line @typescript-eslint/no-unused-expressions
    defaultClick && theme.sounds.push(defaultClick);
  }
  if (hasUseDefaultTriple && !theme.sounds.find((s: any) => s.name === 'triple')) {
    const defaultTripleSound = defaultTheme.sounds.find((s: any) => s.name === 'triple');
    // eslint-disable-next-line @typescript-eslint/no-unused-expressions
  ...
  for (const sound of theme.sounds) {
    if (['triple', 'button-click'].includes(sound.name))
      // @ts-ignore
      sound.src = defaultTheme.sounds.find((s: any) => s.name === sound.name).src;
  }
};
...
export const timestampToUsedTimeString: (time: number) => string = (time) => {
  try {
    ...
    const seconds = ((time - 1000 * 60 * 60 * hours - 1000 * 60 * minutes) / 1000).toFixed(3);
    if (hours) {
...
export const drawImgSrcInCanvas: (params: drawImgSrcInCanvasParams) => Promise<void> = async ({
  imgSrc,
  canvas,
  scale = 1,
}) => {
...
export const canvasToFile: (params: CanvasToFileParams) => Promise<File> = async ({
  canvas,
  fileName,
  maxFileSize,
}) => {
...
export const getFileBase64String: (file: File) => Promise<string> = (file: File) => {
  return new Promise((resolve, reject) => {
